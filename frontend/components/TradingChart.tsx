...
export type Range = '1J' | '1SEM' | '1MOIS' | '3MOIS' | '1AN';
...
interface TradingChartProps {
  ...
  type?: 'stock' | 'crypto';
  height?: number;
  /** When set, the range selector is hidden and this range is used instead */
  rangeOverride?: '1J' | '1SEM' | '1MOIS';
}
...
export default function TradingChart({
  ...
  const [range, setRange]     = useState<Range>(rangeOverride ?? '1MOIS');
  const [loading, setLoading] = useState(true);
  const [error, setError]     = useState('');

  // When rangeOverride changes, update internal range
  useEffect(() => {
  ...
  // Load data when symbol/range/type changes
  ...
  const showRanges = !rangeOverride;

  return (
    <div>
      {/* Range selector — hidden when rangeOverride is provided */}
      {showRanges && (
