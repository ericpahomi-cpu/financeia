...
// ── Chart widget + AI analysis button ────────────────────────────────────────
function InlineChart({ symbol }: { symbol: string }) {
  ...
  const handleAnalyze = async () => {
    if (!containerRef.current || analyzing) return;
    setAnalyzing(true);
    setAnalysis(null);
    setAnalyzeErr(null);

    try {
      // Lazy-load html2canvas (large lib, only pulled when needed)
      ...
      const res = await fetch('/api/analyze-chart', {
        ...
        body:    JSON.stringify({ imageBase64, symbol }),
      });
      ...
      console.error('[analyze-chart]', err);
      setAnalyzeErr("L'analyse a échoué. Réessayez.");
    } finally {
      setAnalyzing(false);
    }
  };
  ...
  return (
    <div className="mt-3 rounded-xl border border-[#e5e7eb] bg-white shadow-sm overflow-hidden">
      ...
      {/* Analyze button */}
      <div className="px-4 py-2 border-t border-[#f3f4f6] flex items-center gap-3">
        <button
          onClick={handleAnalyze}
          disabled={analyzing}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all
            bg-indigo-50 hover:bg-indigo-100 text-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {analyzing ? (
            <>
        ...
        {analysis && !analyzing && (
          <span className="text-[#9ca3af] text-xs">Analyse IA disponible ↓</span>
        )}
      </div>

      {/* Analysis result */}
      {analysis && (
        <div className="px-4 py-3 border-t border-indigo-50 bg-gradient-to-b from-indigo-50/60 to-white">
          ...
          <p className="text-sm text-[#374151] leading-relaxed whitespace-pre-line">{analysis}</p>
        </div>
      )}

      {/* Error */}
      {analyzeErr && (
        <div className="px-4 py-2 border-t border-red-100 text-xs text-red-600">{analyzeErr}</div>
      )}
    </div>
  );
}
